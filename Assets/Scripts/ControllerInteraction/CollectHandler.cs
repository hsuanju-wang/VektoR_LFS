...
public class CollectHandler : MonoBehaviour
{
    ...
    public GameObject rightControllerQuest;
    ...
    private void Collect( GameObject sample)
    {
        ...
        {
            if (rightControllerQuest.GetComponent<TriggerAnim>().isStarted && !rightControllerQuest.GetComponent<TriggerAnim>().isEnd)
            {
                rightControllerQuest.GetComponent<Quest>().EndQuest();
                ControllerOutside.s.dialogueIsEnd = true;
            }
        }
        else if (collectedSamples == 2)
        {
            OutsideSM.s.PlaySampleCollected();
        }
